#requires -Version 2
function Invoke-XPLoadBalance
{
    [cmdletbinding()]

    Param(
		[Parameter(Mandatory = $False)]
		[string()]$ComputerName = (Get-XPWorkstationInfo -ComputerName (Get-DomainPC).Name -WarningAction SilentlyContinue).Where({$_.ApplicationServer -notmatch 'XPA|XPT'}).PSComputerName,
		
		[Parameter(Mandatory = $False)]
		[string()]$ApplicationServers = (Get-ADComputer -Filter "name -like 'xps-web*'").Name
	)

    Begin{}

    Process{
        $AllPCs = $ComputerName | Group-Object -Property @{e = {$_.Substring(0,2)}}

		$Counts = $ApplicationServers | ForEach-Object -Process {[pscustomobject]@{ApplicationServer = $_ ; Count = 0}}

		$Results = Foreach($Location in $AllPCs)
		{
			Foreach($PC in $Location.Group)
			{
				If($PC -match 'TLR')
				{
					$ApplicationServer = $Counts |
					Where-Object -Property ApplicationServer -NE -Value 'XPS-WEB9' |
					Sort-Object -Property Count |
					Select-Object -First 1 -ExpandProperty ApplicationServer
				}
				Else
				{
					$ApplicationServer = $Counts |
				...
				$ArrayPosition = $ApplicationServer[-1] - 49 
        
				$Counts[$ArrayPosition].Count = ++$Counts[$ArrayPosition].Count
			} 
		}

		$BalancedGroups = $Results | Group-Object -Property ApplicationServer

		Foreach($Group in $BalancedGroups)
		{
			Set-XPWorkstationInfo -ComputerName $Group.Group.ComputerName -ApplicationServer $Group.Group.ApplicationServer[0] -WarningAction SilentlyContinue -Confirm:$False
		}
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUEGVnQ/DiW7CmlhOSwxRTjBz4
# hVOgghF4MIIDmDCCAoCgAwIBAgIQFTuOD6CJ+6NFVKZ6+fJQbzANBgkqhkiG9w0B
...
# AQkEMRYEFMtKxpDljoryctmMDBEj2BbQB4sDMA0GCSqGSIb3DQEBAQUABIGAKoCl
# Ofz+tmu89QWeg9J9JJ6Aet4W2xEfmpG0VylzLHaO9n9Ehvtb/fYjnWHhXVAzrWv+
# oct8Vaawedz97COQffsTNoIF9OzNCiRF9G182D4CHrL3MFx8Y2avQlsQ4onAKHob
# NH3RnOqb72B4LNIqOARKzeTe8LMTePa++gjC0tOhggILMIICBwYJKoZIhvcNAQkG
...
# KoZIhvcNAQkDMQsGCSqGSIb3DQEHATAcBgkqhkiG9w0BCQUxDxcNMTYwMTIzMDQz
# MDQ2WjAjBgkqhkiG9w0BCQQxFgQUOPoslvH/y3o01kX4Vvs4Iz4gMPEwDQYJKoZI
# hvcNAQEBBQAEggEAlxN5RZEuF16SZYuWLbW/0ukdNJyiNGSPgjDIwMRQUnqc4Ise
# qAWj3lgLqm9fLDhanXqKtPqeucFYM3xECSziPPIPan83AzxuzKUAGb/UBbKEENGG
# 6gLZVJr2pySSyt1VgpBcAeDqlxiUUv3YYFmwEmu4eq2wnE3gAlWInmFUuyNagm7k
# jZO7GJYVekUNMkG69ymJKNLGtY7iQgqKY9/FoSsAnaOHr1/clfLzx3r8ce5TndIy
# G+80iEhBTdTL/u1oxhVgBxgrq6+UPP5YmabtVaiLq7Hszg3FnuzdwaMJ8xMtEi6L
# XertcmuEBuptPk+4uy72HaL5X8CqhXKKNWdd7Q==
