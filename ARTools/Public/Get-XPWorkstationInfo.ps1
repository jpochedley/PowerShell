#requires -Version 2
function Get-XPWorkstationInfo 
{
    [cmdletbinding()]
        ...
        [scriptblock]$ScriptBlock = {
            ...
            If($null -ne $XPWSSettings)
            {
                ...
                $WSProps = @{
                    ...
                    WorkstationNumber      = $Workstation.WorkstationOptionsViewModel.WorkstationNumber
                    WorkstationDescription = $Workstation.WorkstationOptionsViewModel.WorkstationDescription
                    ApplicationServer      = $Workstation.WorkstationOptionsViewModel.ApplicationServer
                    AssignFromPC           = $Workstation.WorkstationOptionsViewModel.AssignFromPC
                    SSLEnabled             = $Workstation.WorkstationOptionsViewModel.SSLEnabled
                }

                $XPPrintSettings = Get-Item -Path "$Path\*PrinterOptionSettings.xml" -ErrorAction SilentlyContinue |
                Sort-Object -Property LastWriteTime -Descending |
                Select-Object -First 1
            
                If($null -ne $XPPrintSettings)
                {
                    $PrinterObjects = @()
                    [xml]$Printers = Get-Content -Path $XPPrintSettings.fullname -ErrorAction SilentlyContinue
                    Foreach($Printer in $Printers.PrinterOptionsViewModel.PrinterList.XP2Printer)
                    {
                        $PrintProps = [ordered]@{
                            ComputerName       = $env:COMPUTERNAME
                            WorkstationNumber  = $WSProps.WorkstationNumber
                            PrinterNumber      = $Printer.PrinterNumber
                            PrinterDescription = $Printer.PrinterDescription
                        }
                        $PrinterInfo = New-Object -TypeName PSCustomObject -Property $PrintProps
                        $PrinterObjects += $PrinterInfo
                    }
                    $WSProps.Printers = $PrinterObjects
                }
                Else
                {
                    $WSProps.Printers = $null
                }

                $WorkstationInfo = New-Object -TypeName PSCustomObject -Property $WSProps
                $WorkstationInfo
            }
            Else
            {
                Write-Warning -Message "No XP workstation options file found on $env:COMPUTERNAME."
            }
        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwKsYj9cJ7bKxnSzrfY1eX3qG
# euCgghF4MIIDmDCCAoCgAwIBAgIQFTuOD6CJ+6NFVKZ6+fJQbzANBgkqhkiG9w0B
...
# AQkEMRYEFG/iCLS9GOQ21IqE/+L3W9/HwJrMMA0GCSqGSIb3DQEBAQUABIGALukN
# vikUuNt1r6mRe8nzCxIDsl/hu3bsZgm18K95JtHeqarAyWW03HVJbSB4kOwWc8uy
# wa8tOw57NLXVzz4A04Pq+tWSrnitvdbxoqxRJj+MH2zlphO2l4+RxjNKFbxaDQha
# DrWIY/IcNMxwDv1AJsDjR2FOkEylSUGQT19bhPahggILMIICBwYJKoZIhvcNAQkG
...
# MzQ2WjAjBgkqhkiG9w0BCQQxFgQURx0Ntm4UIXPK13NaY3t9DA9QBkEwDQYJKoZI
# hvcNAQEBBQAEggEAE60XMkwFF6wusb73TmuG2AP6q7pF44c6jhlQheRtlBHK/z4v
# tY91WigCIEP4yGGAvdTpPv84Mo1H78SVPdUNoWIl1klQtiNKsl83ZknG/iK+2Jwg
# ASas0holvXBe6fYLroBHOQeqXobbjKAA9sOqqPU9bBGNBkp/4MQtpalAdpixa4zD
# 8GhE5t92xOBEkUzk1IheJx1tMQmnsIYDkA7HESsCzH0JNRrEwIIGQEH76AbjAyju
# kKtDJ5P1Yet70aMqFJvXVCXdxUpUgWrJwbZdMbwRxlqI5es6YeeLA/2pub1okipr
# Tfe/0/dUp7s8G/hj4E0Z6J6+67wEdsm4Et8W9Q==
